...
def should_process_record(
    old_image: Optional[Dict[str, Any]], new_image: Dict[str, Any]
) -> bool:
    """
    ...
    logger.debug(
        "No relevant changes (old_status=%s, new_status=%s), skipping",
        old_status,
        new_status,
    )
    return False
...
def lambda_handler(event: Dict[str, Any], context: Any) -> None:
    ...
    for record in event["Records"]:
        ...
        try:
            ...
            logger.debug(
                "Old image: %s", json.dumps(old_image) if old_image else "None"
            )
            ...
            logger.info(
                "Sending message to SQS for topic '%s' and date '%s' (retry %d)",
                message["topic_id"],
                message["date"],
                message["retry_count"],
            )
            ...
            logger.info(
                "Message sent successfully, MessageId: %s", response.get("MessageId")
            )
