...
def load_messages_from_web(
    topic: Topic, summary_date_str: str, add_to_cache: bool = False
):
    ...
    if topic.max_date >= summary_date_str:
        logger.debug(
            f"Have we fetched before yesterday? = {topic.has_date(before_date_str)}"
        )
    ...
    if messages and add_to_cache:
        try:
            ...
            logger.info(
                f"Stored {len(messages)} messages in DynamoDB for topic {topic.id} on {summary_date_str}"
            )
...
def get_topic(cat, subcat, post) -> Topic:
    ...
    response = topics_table.get_item(Key={"topic_id": topic_id})
    if "Item" in response:
...
def lambda_handler(event: Dict[str, Any], context: Any) -> None:
    ...
    for record in event["Records"]:
        ...
        try:
            logger.info(
                "Processing summary for topic: %s, date: %s", topic_id, date_str
            )
            (cat, subcat, post) = str.split(topic_id, "#")
            topic = Topic(cat, subcat, post)
            load_messages_from_ddb(topic, date_str)
            if not topic.has_date(date_str) or not topic.has_date(
            ...
            if messages:
                try:
                    ...
                    summary_data = {"status": "error", "error": str(e)}
                ...
                if summary_data["success"]:
                    ...
                    new_summary = {
                        ...
                        "error_details": summary_data.get(
